...
int vrpn_get(const char *object, const char *hostname, float pos[3], float orient[16])
{
	...
	{
		printf("Empty or NULL object name was passed into this function.\n");
		return 0;
	}
	if(hostname != NULL && strlen(hostname) == 0)
	{
		printf("Hostname is an empty string.\n");
		return 0;
	}
	...
	if(hostname == NULL)
	{
		...
		{
			printf("Failed to find hostname of VRPN server.\n");
			exit(EXIT_FAILURE);
		}
	...
	else
	{
		...
		{
			delete connection;
			printf("Failed to connect to tracker: %s\n", fullname.c_str());
			return 0;
		}
